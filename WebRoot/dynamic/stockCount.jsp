...
<%
	...
	Element dataInfosEL = analysisJBean.getStockList();
...
<script language="javascript">
	...
	function goOtherPage( pageOffset ){	
		if(pageOffset>0 && pageOffset<=<%=pageTotal%> && pageOffset!=<%=pageOffset%>){
			var url = "<%=tableOrder.getFormatedURL("stockCount.jsp", new String[]{"PAGE_OFFSET"})%>" + "&PAGE_OFFSET=" + pageOffset + "&PAGE_ROW_TOTAL=<%=pageRowTotal%>";
			window.location = url;
		}
	}
	...
	function chgProvince(obj){
		if($(obj).val() != ""){
			$.ajax({
				url:"stockCount.jsp?ACTION=getCity&provinceId=" + $(obj).val(),
			    type: "POST",
	...
	function chgCity(obj){
		if($(obj).val() != ""){
			$.ajax({
				url:"stockCount.jsp?ACTION=getDist&provinceId=" + $("#provinceId").val() + "&cityId=" + $(obj).val(),
			    type: "POST",
			    success:function(retStr){
...
<!--------  Top Begin ------->
	<jsp:include page="/top.jsp" flush="false">
		...
		<jsp:param name="title" value="产品终端销售库存明细统计报表（年  月   日）" />
	</jsp:include>
<!--------  Top End ------->
...
<div style="background:url(../images/main_bg.gif) bottom repeat-x #e7eef8; margin-left:190px; min-height:720px;">
	...
	<table border="0" width="100%" cellspacing="0" class="countTable" >
	  <tr class="listTitle">
	    <td colspan="6">
		<form id="base" name="base" method="post" action="stockCount.jsp">
	     省份：
	        <%=provinceSelect %>
	     城市：
	  ...
	  <tr class="listTitle">
		...
		<td colspan="2" align="center">库存标号</td>
		<td width="15%" rowspan="2" align="center" class="marginListTitle">产品到达时间</td>
		<td width="40%" rowspan="2" align="center" class="marginListTitle">&nbsp;</td>
      </tr>
	  <tr class="listTitle">
	    <td width="15%" align="center">产品外标序号</td>
	    <td width="15%" align="center">产品内标序号</td>
      </tr>
	  <%
	  String className = "";
